...
SESSION_START();
	...
	if((!empty($_SESSION['itfStaffID'])) && (strcmp($_SESSION['itfStaffRole'],"Admin") == 0))
	{
		...
		if(!empty($_GET['action']) && !empty($_GET['id']))
		{
			...
			{
				$type = "Industry";
				//Check if its been approved before
				if($_GET['action'] == "Approve")
				{
					
					//Check if its been approved before
					$sql = "SELECT * FROM tblIndustries WHERE industryRegistrationNumber='". $_GET['id'] ."'";
					...
					//get Application info from tblApplications tables
					$sql = "SELECT * FROM tblApplications WHERE applicationType='". $_GET['type'] ."' AND applicationNumber='". $_GET['id'] ."'";
					$result = mysqli_query($con, $sql) or die(mysqli_error($con));
					...
					if(mysqli_num_rows($result) > 0)
					{
						...
						{
							$email = $row['applicationEmail'];
							$recNumber = $row['applicationNumber'];
							$name = $row['applicationName'];
							$office = $row['applicationOffice'];
							
						}
						$username = $email;
						if($alreadyApproved)
						{
							//if once approved, we update the tblSchools otherwise we copy the info. from tblApplications
							$sql = "UPDATE tblIndustries SET industryStatus = '1' WHERE industryRegistrationNumber='". $recNumber ."'";
							mysqli_query($con, $sql) or die(mysqli_error($con));
							
							$success = true;
						}else{
							require_once '../randomPassword.php';
							$pass = randomPassword(9);
							$passHash = password_hash($pass, PASSWORD_DEFAULT);
							
							$sql = "INSERT INTO tblIndustries (industryRegistrationNumber, industryName, industryOffice,industryStatus)
								VALUES ('". $recNumber ."','". $name ."','" . $office ."','1')";
							mysqli_query($con, $sql) or die(mysqli_error($con));
							
							$sql = "INSERT INTO tblIndustryStaff (industryRegistrationNumber, industryStaffEmail, industryStaffPassword, industryStaffDesignation,idustryStaffStatus)
								VALUES ('". $recNumber ."','". $email ."','" . $passHash ."','Admin','1')";
							mysqli_query($con, $sql) or die(mysqli_error($con));
						...
						if($success )
						{
							$sql = "UPDATE tblApplications SET applicationStatus = '1' WHERE applicationNumber='". $recNumber ."'";
							mysqli_query($con, $sql) or die(mysqli_error($con));
							/*
							if( mysqli_affected_rows($con) > 0 )
							{
								$errorMessage = "Successful Approval of Application!!!";
								
							}else{
								$errorMessage = "Application Approval Unsuccessful, Try Again!!!";
							}
							*/
						}else{
							
						}
						$headers = "From: itfsiwes@gmail.com \r\n MIME-Version: 1.0 \r\n Content-type:text/html;charset=UTF-8 \r\n";
						if(mail($email, "IFT SIWES System Application Approval Notification", "Dear Sir/Ma, \nThank for having express your interest in contributing your quota towards making Nigeria great for our future leaders. This mail is to notify you on the reception of your request to join SIWES Scheme as an ". $type . ". Our dedicated team have now worked on your application and found your ".$type." worthy of being part of this great mission. Therefore, your login credentials are as follows: \n\nUsername: ".$username." \nPassword: ".$pass." \n\nThank you and once again and welcome. \n\n\nSincerely, \nOur ITF Team.", $headers))
						{
					...
					$errorMessage = "Account Deactivation/Deletion Unsuccessful, Try Again!!!";
					$sql = "DELETE FROM tblApplications WHERE applicationNumber='". $_GET['id'] ."'";
					mysqli_query($con, $sql) or die(mysqli_error($con));
					
					$errorMessage = "Account Successfully Deactivated/Deleted!!!";
						
					
				}elseif(($_GET['action'] == "Restore") || ($_GET['action'] == "Suspend")){
					$action = 1;
					...
					$sql = "UPDATE tblApplications SET applicationStatus = '".$action."' WHERE applicationNumber='". $_GET['id'] ."'";
					mysqli_query($con, $sql) or die(mysqli_error($con));
					
					$sql = "UPDATE tblIndustries SET industryStatus = '".$action."' WHERE industryRegistrationNumber='". $_GET['id'] ."'";
					mysqli_query($con, $sql) or die(mysqli_error($con));
					
					if( mysqli_affected_rows($con) > 0 )
					{
						if($action == 1){
							$errorMessage = "Successfully Restored!!!";
						}else{
							$errorMessage = "Successfully Suspended!!!";
						}
					}else{
						if($action == 1){
							$errorMessage = "Unsuccessful Restore, Try Again!!!";
						}else{
							$errorMessage = "Unsuccessful Suspension, Try Again!!!";
						}
			...
			}elseif($_GET['type'] == '2')
			{
				...
				if($_GET['action'] == "Approve")
				{
					...
					//Check if its been approved before
					$sql = "SELECT * FROM tblSchools WHERE schoolIdentificationNumber='". $_GET['id'] ."'";
					$result = mysqli_query($con, $sql) or die(mysqli_error($con));
					...
					if(mysqli_num_rows($result) > 0)
					{
						...
						{
							//if once approved, we update the tblSchools otherwise we copy the info. from tblApplications
							$sql = "UPDATE tblSchools SET schoolStatus = '1' WHERE schoolIdentificationNumber='". $recNumber ."'";
							mysqli_query($con, $sql) or die(mysqli_error($con));
							...
							$sql = "INSERT INTO tblSchools (schoolIdentificationNumber, schoolName, schoolOffice, schoolStatus)
								VALUES ('". $recNumber ."','". $name ."','" . $office ."','1')";
							mysqli_query($con, $sql) or die(mysqli_error($con));
							
							$sql = "INSERT INTO tblSchoolStaff (schoolIdentificationNumber, schoolStaffEmail, schoolStaffPassword, schoolStaffDesignation, schoolStaffStatus)
								VALUES ('". $recNumber ."','". $email ."','" . $passHash ."','Admin','1')";
							mysqli_query($con, $sql) or die(mysqli_error($con));
					...
					$sql = "UPDATE tblSchools SET schoolStatus = '".$action."' WHERE schoolIdentificationNumber='". $_GET['id'] ."'";
					mysqli_query($con, $sql) or die(mysqli_error($con));
    ...
    <div id="apDiv36">
      ...
      <div id="apDiv63">
		...
		<h1 class="title">
			<?php 
				...
				if($_GET['type'] == '1')
				{
					$type = "Industries Applications";
				}else
				{
					$type = "Institutions Applications";
				}
				print($type);
			?>
		</h1>
		...
		<div class='container'>
			...
			<form id="form" class="form" method="POST" onsubmit="return ValidateSubmission();" action="siwesDailyReport.php" enctype="multipart/form-data">
				...
				<table border='1' width='100%' align='left'>
					<tr><th width='35%'> Names </th><th width='10%'> Reg. Number </th><th width='35%'> Address </th><th width='20%'> Action </th></tr>
					<?php
						if(!empty($_GET['type']))
						{
							$output ="";
							...
							$sql = "SELECT applicationType, applicationNumber, applicationName, applicationEmail, applicationOffice, applicationStatus FROM tblApplications WHERE applicationType='". $_GET['type']."'";
							$result = mysqli_query($con, $sql);//or die(mysqli_error($con));
							...
							if( mysqli_num_rows($result) > 0 )
							{
								...
								while($row = mysqli_fetch_assoc($result))
								{
									...
									{
										$output .= "<tr><td width='35%'>".$row['applicationName']."</td>".
											"<td width='10%'>".$row['applicationNumber'] ."</td>".
											"<td width='35%'>".$row['applicationOffice'] ."</td>".
											"<td width='20%'><a href='viewApproveApplications.php?type=".$_GET['type']."&id=".$row['applicationNumber'] ."&action=Approve'>Approve</a>||
											<a href='viewApproveApplications.php?type=".$_GET['type']."&id=".$row['applicationNumber'] ."&action=Delete'>Delete</a></td></tr>";
									}elseif($row['applicationStatus'] == '1'){
										
										$approvedApp .="<tr><td width='35%'>".$row['applicationName']."</td>".
											"<td width='10%'>".$row['applicationNumber'] ."</td>".
											"<td width='35%'>".$row['applicationOffice'] ."</td>".
											"<td><a href='viewApproveApplications.php?type=".$_GET['type']."&id=".$row['applicationNumber'] ."&action=Suspend'>Suspend Approval</a></td></tr>";
									}else{
										$suspended .="<tr><td width='35%'>".$row['applicationName']."</td>".
											"<td width='10%'>".$row['applicationNumber'] ."</td>".
											"<td width='35%'>".$row['applicationOffice'] ."</td>".
											"<td><a href='viewApproveApplications.php?type=".$_GET['type']."&id=".$row['applicationNumber'] ."&action=Restore'>Restore Approval</a></td></tr>";
								...
								if(!empty($output))
								{
									print("<tr><td colspan='3'><a href='#'><div class='menuButton'>New Applications</div></a></td></tr>");
									print($output);
								}else{
									print("<tr><td colspan='3'><a href='#'><div class='menuButton'>No New Applications Found For ".$type."</div></a></td></tr>");
									
								}
								
								if(!empty($approvedApp))
								{
									print("<tr><td colspan='3'><a href='#'><div class='menuButton'>Approved Applications</div></a></td></tr>");
									print($approvedApp);
								}else{
									print("<tr><td colspan='3'><a href='#'><div class='menuButton'>No Approved Applications Found For ".$type."</div></a></td></tr>");
									
								}
								if(!empty($suspended))
								{
									print("<tr><td colspan='3'><a href='#'><div class='menuButton'>Suspended Applications</div></a></td></tr>");
									print($suspended);
								}else{
									print("<tr><td colspan='3'><a href='#'><div class='menuButton'>No Suspended Applications Found For ".$type."</div></a></td></tr>");
      ...
      <div id="apDiv65"  style="border:1px #666666 solid;">
		...
		<div class='container'>
			...
			<a href='viewApproveApplications.php?type=1'><div class='menuButton'>Industries Applications</div></a>
			
			<a href='viewApproveApplications.php?type=2'><div class='menuButton'>Institutions Applications</div></a>
			<a href='adminDashboard.php'><div class='menuButton'>Main Dashboard</div></a>
