...
SESSION_START();
	$errorMessage ='';
	if((!empty($_SESSION['schoolStaffID'])) && (strcmp($_SESSION['schoolStaffRole'],"Staff") == 0))
	{	
		if(isset($_POST['btnSend'])){
			...
			$sql = "UPDATE tblDailyActivity SET  schoolSupervisorComment='". $_POST['txtSupervisorComment'] ."' WHERE studentMatricNumber='". $_POST['studentmatricNumber'] ."' AND activityDate='". $_POST['studentmatricNumber'] ."'";
			mysqli_query($con, $sql); // or die(mysqli_error($con));
			
			if( mysqli_affected_rows($con) > 0 )
			{
				$errorMessage = "Successfully Change Password";
  ...
  <div id="mainContent">
    ...
    <div id="apDiv36">
			...
			<table style="table-layout: fixed; width: 100%; max-height:50px; border:1">
				<tr>
					<td width='50%' colspan='3' align='center'>
						<?php
							...
							{
								require_once '../connection.php';
								$sql = "SELECT studentFirstName, studentLastName FROM tblStudents WHERE studentMatricNumber='". $_GET['studentID']."'";
				...
				<tr>
					<?php
						...
						if((!empty($_GET['studentMatricNumber'])) && (!empty($_GET['activityDate'])) && (empty($_GET['comment'])))
						{
							require_once '../connection.php';
							$sql = "SELECT activityDescription, activitySnapshot, schoolSupervisorComment FROM tblDailyActivity WHERE studentMatricNumber='". $_GET['studentMatricNumber'] ."' AND activityDate='". $_GET['activityDate'] ."'";
							$result = mysqli_query($con, $sql);//or die(mysqli_error($con));
							if( mysqli_num_rows($con) > 0 )
							{
								while($row = mysqli_fetch_assoc($result))
								{
									...
									print("<td width='50%'height='10px'style='overflow-y:scroll;font-size:0.7em; word-wrap: break-word;'>
											<div class='cellContainer'>".$row['activityDescription']."</div></td><td width='30%'height='10px'>"
											."<a href='ActivityGallery/".$row['activitySnapshot']."</a><?/td><td width='20%'height='10px'align='center'>
												<textarea id='txtSupervisorComment' name='txtSupervisorComment' placeholder='Supervisor Comment/Recommendation'></textarea>
												<input type='hidden' name='studentMatricNumber' value='". $_GET['studentMatricNumber'] ."' /><input type='hidden' name='activityDate' value='". $_GET['activityDate'] ."' />
												<button class='button' name='btnSend' id='btnSend'>Send</button><button class='button' name='btnSend' id='btnSend'>Cancel</button>
											</td>");
								}
								
							}
						
						}elseif((!empty($_GET['studentMatricNumber'])) && (!empty($_GET['comment'])))
						{
							require_once '../connection.php';
							if($_GET['comment'] == 'ift')
							{
								$sql = "SELECT itfSupervisorComment FROM tblDailyActivity WHERE studentMatricNumber='". $_GET['studentMatricNumber'] ."' AND activityDate='". $_GET['activityDate'] ."'";
								$result = mysqli_query($con, $sql);//or die(mysqli_error($con));
								if( mysqli_num_rows($con) > 0 )
								{
									while($row = mysqli_fetch_assoc($result))
									{
										print($row['itfSupervisorComment']);
									}
									
								}
							}else{
								$sql = "SELECT industrialSupervisorComment FROM tblDailyActivity WHERE studentMatricNumber='". $_GET['studentMatricNumber'] ."' AND activityDate='". $_GET['activityDate'] ."'";
								$result = mysqli_query($con, $sql);//or die(mysqli_error($con));
								if( mysqli_num_rows($con) > 0 )
								{
									while($row = mysqli_fetch_assoc($result))
									{
										print($row['industrialSupervisorComment']);
									}
			...
			<form id="form" class="form" method="POST" onsubmit="return ValidateSubmission();" action="siwesDailyReport.php" enctype="multipart/form-data">
				...
				<table border='1' width='100%' align='left'>
					<?php
						...
						{
							require_once '../connection.php';
							$sql = "SELECT activityDate, industrialSupervisorComment, itfSupervisorComment FROM tblDailyActivity WHERE studentSupervisor='". $_SESSION['staffID'] ."' AND studentCompleted = 0";
							$result = mysqli_query($con, $sql);//or die(mysqli_error($con));
							if( mysqli_num_rows($con) > 0 )
							{
								while($row = mysqli_fetch_assoc($result))
								{
									print("<tr><td width='20%'><a href='viewCurrentStudents.php?studentMatricNumber=".
											$_GET['studentMatricNumber']."&activityDate=".$row['activityDate']."'>".$row['activityDate'] ."</a></td>".
											"<td width='40%' style='overflow:hide;'><a href='viewCurrentStudents.php?studentMatricNumber=".
											$_GET['studentMatricNumber']."&activityDate=".$row['activityDate']."&comment=Industrial' target='_blank'>".$row['industrialSupervisorComment']."</a></td>".
											"<td width='40%' style='overflow:hide;'><a href='viewCurrentStudents.php?studentMatricNumber=".
											$_GET['studentMatricNumber']."&activityDate=".$row['activityDate']."&comment=ift' target='_blank'>".$row['itfSupervisorComment']."</td></tr>");
      ...
      <div id="apDiv65"  style="border:1px #666666 solid;">
		...
		<div class='container'>
			<div class="title">Student On-Supervision</div>
			<?php  
				require_once '../connection.php';
				$sql = "SELECT studentMatricNumber FROM tblStudents WHERE studentSupervisor='". $_SESSION['staffID'] ."' AND studentCompleted = 0";
				$result = mysqli_query($con, $sql);//or die(mysqli_error($con));
				if( mysqli_num_rows($con) > 0 )
				{
					while($row = mysqli_fetch_assoc($result))
					{
						print("<a href='viewCurrentStudents.php?studentID=".$row['studentMatricNumber']."'><div class='menuButton'>".$row['studentMatricNumber']."</div></a>");
			...
			?>
			<a href='staffDashboard.php'><div class='menuButton'>Return To Dashboard</div></a>
