...
SESSION_START();
		...
		if(isset($_POST['btnSubmitButton'])){
			...
			SESSION_UNSET(); //Initialize all sessions
			
			if(empty($_POST['txtUsername']))
			{
				$isError = true;
			}
			if(empty($_POST['cboSelectLogin']))
			{
				$isError = true;
			}
			...
			if (!($isError))
			{
				...
				if($_POST['cboSelectLogin'] == "STD") 
				{
					//STUDENT LOGIN Option
					
					require_once 'connection.php';
					$sql = "SELECT studentMatricNumber, studentFirstName, studentMiddleName, studentLastName,studentPassword FROM tblStudents WHERE studentMatricNumber='". $_POST['txtUsername'] ."' OR studentEmail ='". $_POST['txtUsername'] ."'";
					...
					if(mysqli_num_rows($result) > 0 )
					{
						...
						while($row = mysqli_fetch_assoc($result)){
							...
							if(password_verify($_POST['txtPassword'], $row['studentPassword']))
							{
							
								$_SESSION['firstName'] = $row['studentFirstName'];
								$_SESSION['middleName'] = $row['studentMiddleName'];
								$_SESSION['lastName'] = $row['studentLastName'];
								$_SESSION['matricNumber'] = $row['studentMatricNumber'];
								
								header('location:studentDashboard.php');
							}else{
								$errorMessage = "Student: Wrong Username or Password!!!";
							}
						}
					}else{
						$errorMessage = "Student: Username Not Found!!!";
					}
				}elseif($_POST['cboSelectLogin'] == "INDStaff")
				{
					//Industry Staff LOGIN Option
						
					require_once 'connection.php';
					$sql = "SELECT industryRegistrationNumber, industryStaffNumber, industryStaffFirstName, industryStaffMiddleName, industryStaffLastName, industryStaffDesignation, industryStaffPassword FROM tblIndustryStaff WHERE industryStaffNumber='". $_POST['txtUsername'] ."' OR industryStaffEmail ='". $_POST['txtUsername']."'  AND industryStaffStatus = '1' AND industryStaffDesignation='Staff'";
					$result = mysqli_query($con, $sql) or die(mysqli_error($con));
					...
					if(mysqli_num_rows($result) > 0 )
					{
						...
						while($row = mysqli_fetch_assoc($result)){
							...
							$_SESSION['industryID'] = $row['industryRegistrationNumber'];
							$_SESSION['industryStaffNumber'] = $row['industryStaffNumber'];
							$_SESSION['industryStaffFirstName'] = $row['industryStaffFirstName'];
							$_SESSION['industryStaffLastName'] = $row['industryStaffLastName'];
							$_SESSION['industryStaffDesignation'] = $row['industryStaffDesignation'];
							
							if(password_verify($_POST['txtPassword'], $row['industryStaffPassword']))
							{
				...
				}elseif($_POST['cboSelectLogin'] == "INDAdmin")
				{
					//Industry Admin LOGIN Option
					...
					if(mysqli_num_rows($result) > 0 )
					{
						...
						while($row = mysqli_fetch_assoc($result)){
							...
							if(password_verify($_POST['txtPassword'], $row['industryStaffPassword']))
							{
								header('location:institutionAdminDashboard.php');
							}else{
								$errorMessage = "Wrong Username or Password!!!";
							}
				...
				}elseif($_POST['cboSelectLogin'] == "ISTStaff")
				{
					//Institution STAFF LOGIN Option
					//print("Here !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");	
					require_once 'connection.php';
					$sql = "SELECT schoolIdentificationNumber, schoolStaffRegistrationNumber, schoolStaffFirstName, schoolStaffMiddleName, schoolStaffLastName, schoolStaffDesignation, schoolStaffPassword FROM tblSchoolStaff WHERE schoolStaffRegistrationNumber='". $_POST['txtUsername'] ."' OR schoolStaffEmail ='". $_POST['txtUsername'] ."'  AND schoolStaffStatus = '1' AND schoolStaffDesignation='Staff'";
					$result = mysqli_query($con, $sql) or die(mysqli_error($con));
					...
					if(mysqli_num_rows($result) > 0 )
					{
						...
						while($row = mysqli_fetch_assoc($result)){
							...
							if($row['schoolStaffRegistrationNumber'] != "")
							{
								$_SESSION['schoolStaffID'] = $row['schoolStaffRegistrationNumber'];
							}else{
								$_SESSION['schoolStaffID'] = $_POST['txtUsername'];
							}
							
							$_SESSION['schoolID'] = $row['schoolIdentificationNumber'];
							$_SESSION['schoolStaffRole'] = $row['schoolStaffDesignation'];
							
							if(password_verify($_POST['txtPassword'], $row['schoolStaffPassword']))
							{
								header('location:institution/staffDashboard.php');
							}else{
								$errorMessage = "Wrong Username or Password!!!";
							}
				...
				}elseif($_POST['cboSelectLogin'] == "ISTAdmin")
				{
					//Institution Admin LOGIN Option
					$errorMessage = "Here!!!";
					require_once 'connection.php';
					$sql = "SELECT schoolIdentificationNumber, schoolStaffRegistrationNumber, schoolStaffFirstName, schoolStaffMiddleName, schoolStaffLastName, schoolStaffPassword, schoolStaffDesignation FROM tblSchoolStaff WHERE schoolIdentificationNumber='". $_POST['txtUsername'] ."' OR schoolStaffEmail ='". $_POST['txtUsername']  ."'  AND schoolStaffStatus = '1' AND schoolStaffDesignation='Admin'";
					$result = mysqli_query($con, $sql) or die(mysqli_error($con));
					...
					if(mysqli_num_rows($result) > 0 )
					{
						...
						while($row = mysqli_fetch_assoc($result)){
							...
							if(password_verify($_POST['txtPassword'], $row['schoolStaffPassword']))
							{
								header('location:institution/institutionAdminDashboard.php');
							}else{
								$errorMessage = "Wrong Username or Password!!!";
							}
				...
				}elseif(($_POST['cboSelectLogin'] == "ITFAdmin") || ($_POST['cboSelectLogin'] == "ITFStaff"))
				{
					//ITF Staff LOGIN Option
					$sql ="";
					
					
					if($_POST['cboSelectLogin'] == "ITFAdmin") //Admin Option
					{
						
						$sql = "SELECT staffFirstName, staffLastName, staffEmail, staffNumber, staffDesignation, staffPassword FROM tblStaff WHERE staffNumber='". $_POST['txtUsername'] ."' OR staffEmail ='". $_POST['txtUsername']  ."' AND staffStatus = '1' AND staffDesignation='Admin'";
					}else{ ///Staff Option
						$sql = "SELECT staffFirstName, staffLastName, staffEmail, staffNumber, staffDesignation, staffPassword FROM tblStaff WHERE  staffNumber='". $_POST['txtUsername'] ."' OR staffEmail ='". $_POST['txtUsername']   ."'  AND staffStatus = '1' AND staffDesignation='Staff'";
					}
					...
					if(mysqli_num_rows($result) > 0 )
					{
						...
						while($row = mysqli_fetch_assoc($result))
						{
							$_SESSION['itfStaffName'] =  $row['staffLastName'] . " " .$row['staffFirstName'];
							$_SESSION['itfStaffID'] = $row['staffNumber'];
							$_SESSION['itfStaffRole'] = $row['staffDesignation'];
							//print($_POST['cboSelectLogin'] ." : " .$_SESSION['itfStaffRole']);
							
							if($_POST['cboSelectLogin'] == "ITFAdmin") //Admin Option
							{
								
								if(password_verify($_POST['txtPassword'], $row['staffPassword']))
								{
									header('location:itf/adminDashboard.php');
								}else{
									$errorMessage = "Wrong Username or Password!!!";
								}
...
<script type="text/javascript">
	function ValidateSubmission()
	{
		...
		if((Selectedvalue == '') ) {
			...
			errorMessage = "Select Login Option!!!";
		}
		if((document.getElementById('txtUsername').value == '') ) {
			
			errorMessage = "Supply Username!!!";
		}else if((document.getElementById('txtPassword').value == '') ) {
			errorMessage = "Supply Password!!!";
		}else if((document.getElementById('txtPassword').value == '') ) {
			errorMessage = "Supply Password!!!";
		}
		if(errorMessage != ""){
  ...
  <div id="mainContent">
	...
	<li>
      ...
      <div id="apDiv65"  style="border:1px #666666 solid;">
		<label for='name'><a href='siwesApplication.php'><div class='menuButton'>SIWES Inclussion Request</div></a></label>
		<p>&nbsp;</p>
		<div class='shortDashboard'>
			<h1 class="title">SIWES Login</h1>
			<form id="siwesHome" name="siwesHome" class="form" onsubmit="return ValidateSubmission();" action="siwesHome.php" method="POST">
				...
				<select id="soflow"  name='cboSelectLogin' tabindex="1">
				  ...
				  <option value='ITFAdmin'>ITF Admin</option>
				  <option value='ITFStaff'>ITF Staff</option>
				  <option value='ISTAdmin'>Institution Admin</option>
				  <option value='ISTStaff'>Institution Staff</option>
				  <option value='INDAdmin'>Industry Admin</option>
				  <option value='INDStaff'>Industry Staff</option>
				  <option value='STD'>Student</option>
				  
				</select>
				<input type="text" placeholder="Username Here" id="txtUsername" name="txtUsername" tabindex="1"  />
			
				
				<input type="password" placeholder="Password Here" id="txtPassword" name="txtPassword" tabindex="2"/>
				
					<button class='button' id="demo" name='btnSubmitButton'>Submit</button> <button class='button' id="demo" name='btnCancelButton'>Cancel</button> 
					<label for='name'><a href='passwordRecovery.php'>Forgot Password? Recover Here!!!</a></label>
