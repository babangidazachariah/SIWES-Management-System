...
SESSION_START();
	...
	if((!empty($_SESSION['itfStaffID'])) && (strcmp($_SESSION['itfStaffRole'],"Admin") == 0))
	{
		...
		if(!empty($_GET['action']) && !empty($_GET['id']))
		{
			...
			{
				$sql = "UPDATE tblBanks SET bankStatus ='1' WHERE bankID=". $_GET['id'];
				$result = mysqli_query($con, $sql) or die(mysqli_error($con));
				
				
			}elseif($_GET['action'] == 'Suspend')
			{
				$sql = "UPDATE tblBanks SET bankStatus ='2' WHERE bankID=". $_GET['id'];
				$result = mysqli_query($con, $sql) or die(mysqli_error($con));
				
				
			}elseif($_GET['action'] == 'Delete')
			{
				$sql = "DELETE FROM tblBanks WHERE bankID=". $_GET['id'];
				$result = mysqli_query($con, $sql) or die(mysqli_error($con));
			...
			if(mysqli_affected_rows($con) > 0)
			{
				...
				$errorMessage ='Unable To Complete Task, Try Again!!!';
			}
		}
	}else{
    ...
    <div id="apDiv36">
      ...
      <div id="apDiv63">
		...
		<div class='container'>
			...
			<form id="form" class="form" method="POST" onsubmit="return ValidateSubmission();" action="viewEditBanks.php" enctype="multipart/form-data">
				<label id='error' class='error'><?php print($errorMessage); ?></label>
					
				<table border='1' width='100%' align='left'>
					<tr><th width='40%'>Bank Names </th><th width='15%'>Status </th><th width='45%'> Action </th></tr>
					<?php
						...
						$sql = "SELECT * FROM tblBanks ORDER BY bankName";
						$result = mysqli_query($con, $sql);//or die(mysqli_error($con));
						if( mysqli_num_rows($result) > 0 )
						{
							while($row = mysqli_fetch_assoc($result))
							{
								...
								$output .= "<tr><td>".$row['bankName']."</td>
									<td>".$status."</td>".
									"<td><a href='viewEditBanks.php?id=".$row['bankID'] ."&".$action ."</a> ||
									
									<a href='viewEditBanks.php?id=".$row['bankID'] ."&action=Delete'>Delete</a></td></tr>";
							...
							if(!empty($output))
							{
								...
								print("<tr><td colspan='3'><a href='#'><div class='menuButton'>No Banks In Record</div></a></td></tr>");
