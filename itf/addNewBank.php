	...
	if((!empty($_SESSION['itfStaffID'])) && (strcmp($_SESSION['itfStaffRole'],"Admin") == 0))
	{
		...
		if(isset($_POST['btnSubmitButton']))
		{
			...
			if (!($isError))
			{
				...
				$sql = "SELECT * FROM tblBanks WHERE bankName='". $_POST['txtBankName'] ."'";
				$result = mysqli_query($con, $sql) or die(mysqli_error($con));
				
				if(mysqli_num_rows($result) < 1)
				{
					
					$sql = "INSERT INTO tblBanks (bankName, bankStatus) 
															VALUES ('".$_POST['txtBankName']."','1')";
					mysqli_query($con, $sql) or die(mysqli_error($con));
					
					$errorMessage = "Successfully Added.";
					header("location:viewEditBanks.php");
				}else{
    ...
    <div id="apDiv36">
      ...
      <div id="apDiv63">
		...
		<div class='shortContainer'>
			...
			<form id="form" class="form" action="addNewBank.php" onsubmit="return ValidateSubmission();" method="POST">
				<label id='error' class='error'><?php print($errorMessage); ?></label>
				
				<input type="text" placeholder="Bank Name Here" id="txtBankName" name="txtBankName" tabindex="4"/>
