...
SESSION_START();
	...
	if((!empty($_SESSION['schoolStaffID'])) && (strcmp($_SESSION['schoolStaffRole'],"Admin") == 0))
	{
		...
		if(!empty($_GET['action']) && !empty($_GET['id']))
		{
			...
			{
				$sql = "UPDATE tblSchoolStaff SET schoolStaffStatus = '0' WHERE schoolStaffRegistrationNumber='". $_GET['id'] ."'";
				...
				$sql = "UPDATE tblSchoolstaff SET schoolStaffStatus = '1' WHERE schoolStaffNumber='". $_GET['id'] ."'";
				mysqli_query($con, $sql); // or die(mysqli_error($con));
				...
				$sql = "UPDATE tblSchoolStaff SET  schoolStaffDesignation='". $_GET['action'] ."' WHERE schoolStaffNumber='". $_GET['id'] ."'";
				mysqli_query($con, $sql); // or die(mysqli_error($con));
			...
			<form id="form" class="form" method="POST" onsubmit="return ValidateSubmission();" action="siwesDailyReport.php" enctype="multipart/form-data">
				...
				<table border='1' width='100%' align='left'>
					...
					<?php
							...
							$sql = "SELECT schoolStaffFirstName, schoolStaffLastName, schoolStaffRegistrationNumber, schoolStaffEmail, schoolStaffStatus, schoolStaffDesignation FROM tblSchoolStaff";
							$result = mysqli_query($con, $sql);//or die(mysqli_error($con));
							if( mysqli_num_rows($result) > 0 )
							{
								while($row = mysqli_fetch_assoc($result))
								{
									if(($row['schoolStaffDesignation'] == "Admin") && ($row['schoolStaffStatus'] == '1'))
									{
										$adminSet .= "<tr><td width='50%'>".$row['schoolStaffFirstName']." ".$row['schoolStaffLastName'] ."</td>".
											"<td width='15%'>".$row['schoolStaffDesignation'] ."</td>".
											"<td width='35%'><a href='viewEditStaffAdmin.php?id=".$row['schoolStaffRegistrationNumber'] ."&action=Staff'>Make Staff</a>||
											<a href='viewEditStaffAdmin.php?id=".$row['schoolStaffRegistrationNumber'] ."&action=Deactivate'>Deactivate</a></td></tr>";
									}elseif(($row['schoolStaffDesignation'] == "Staff") && ($row['schoolStaffStatus'] == '1'))
									{
										
										$staffSet .="<tr><td width='50%'>".$row['schoolStaffFirstName']." ".$row['schoolStaffLastName'] ."</td>".
											"<td width='15%'>".$row['schoolStaffDesignation'] ."</td>".
											"<td width='35%'><a href='viewEditStaffAdmin.php?id=".$row['schoolStaffRegistrationNumber'] ."&action=Admin'>Make Admin</a>||
											<a href='viewEditStaffAdmin.php?id=".$row['schoolStaffRegistrationNumber'] ."&action=Deactivate'>Deactivate</a></td></tr>";
									}else{
										
										$deactivatedStaff .="<tr><td width='50%'>".$row['schoolStaffFirstName']." ".$row['schoolStaffLastName'] ."</td>".
											"<td width='15%'>".$row['schoolStaffDesignation'] ."</td>".
											"<td width='35%'><a href='viewEditStaffAdmin.php?id=".$row['schoolStaffRegistrationNumber'] ."&action=Activate'>Activate</a></td></tr>";
