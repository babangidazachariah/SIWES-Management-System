...
SESSION_START();
	...
	if((!empty($_SESSION['schoolStaffID'])) && (strcmp($_SESSION['schoolStaffRole'],"Admin") == 0))
	{	
		if(isset($_POST['btnSend'])){
			...
			$sql = "UPDATE tblsTUDENTS SET  StudentSupervisor='". $_POST['cboSupervisor'] ."', StudentMatricNumber='". $_POST['txtMatricNumber']."', StudentEmail='". $_POST['txtEmail'] ."' WHERE studentMatricNumber='". $_POST['studentmatricNumber'] ."' AND schoolID='". $_SESSION['schoolID'] ."'";
			...
			if( mysqli_affected_rows($con) > 0 )
			{
				...
				mail($_POST['txtEmail'], "IFT SIWES System Notification", "Dear Sir/Ma, \nThis is to notify you of changes is being made on you SIWES account to facilitate your access\n\nHowever, your login credentials remain same except: \n\nUsername: ".$_POST['txtEmail']." \nPassword is the same as before. \n\nThank you and once again and welcome. \n\n\nSincerely, \nOur ITF Team.", $headers);
						
				
			}else{
				$errorMessage = "Changes Unsuccessful, Try Again!!!";
			}
		}
	}else{
...
<script type='text/Javascript'>
	...
	function ValidateSubmission()
	{
		...
		if((Selectedvalue == '') ) {
			...
			errorMessage = "Select Supervisor!!!";
		}
		//alert(Selectedvalue);
		if((document.getElementById('txtMatricNumber').value == '') ) {
			errorMessage = "Type Student's Matric Number!!!";
		}
		
		if((document.getElementById('txtEmail').value == '') ) {
			errorMessage = "Type Student's Email!!!";
		}
  ...
  <div id="mainContent">
    ...
    <div id="apDiv36">
      ...
      <div id="apDiv63">
		...
		<h1 class="title">View/Edit Students</h1>
		<div class='outputRibbon'>
			<form id="form" class="form" method="POST" onsubmit="return ValidateSubmission();" action="viewEditStudents.php">
				<label id='error' class='error'><?php print($errorMessage); ?></label>
			
			<table style="table-layout: fixed; width: 100%; max-height:50px; border:1">
				<tr>
					<td width='50%' colspan='3' align='center'>
						<?php
				
							//When the Matric Number of a student is clicked from the right-pane, the student name is displayed on title bar of the ribbon
							if(!empty($_GET['studentID']))
							{
								require_once '../connection.php';
								$sql = "SELECT studentFirstName, studentLastName FROM tblStudents WHERE studentMatricNumber='". $_GET['studentID']."' AND schoolID='". $_GET['schoolID'] ."'";
								$result = mysqli_query($con, $sql);//or die(mysqli_error($con));
								if( mysqli_num_rows($con) > 0 )
								{
									while($row = mysqli_fetch_assoc($result))
									{
										print($row['studentFirstName'] ." ".$row['studentLastName']." {".$_GET['studentID']."}");
										
									}
									
								}else{
									print("<a href='#'><div class='menuButton'>No Records Found</div></a>");
								}
				...
				<tr>
					<?php
						...
						//When the date of an activity is ckicked, the day's activity details are displayed in the outputRibbon
						
						if((!empty($_GET['studentID'])) && (!empty($_GET['action'])) && (!empty($_GET['schoolID'])))
						{
							require_once '../connection.php';
							$supervisors = '';
							$sql = "SELECT * FROM tblSchoolStaff WHERE schoolIdentificationNumber='". $_GET['schoolID'] ."' AND schoolStaffDesignation='Staff' AND schoolStaffStatus='1'";
							$result = mysqli_query($con, $sql);//or die(mysqli_error($con));
							if( mysqli_num_rows($con) > 0 )
							{
								while($row = mysqli_fetch_assoc($result))
								{
									if(!empty($_GET['supervisorID']) && (strcmp($_GET['supervisorID'],$row['studentSupervisor']) == 0))
									{
										$supervisors .= "<option value='".$row['schoolStaffRegistrationNumber']."'>" .$row['schoolStaffFirstName']. " " .$row['schoolStaffLastName']."</option>";
										
									}else{
										$supervisors .= "<option value='".$row['schoolStaffRegistrationNumber']."' selected>" .$row['schoolStaffFirstName']. " " .$row['schoolStaffLastName']."</option>";
									}
								}
							}
							$sql = "SELECT * FROM tblStudents WHERE studentMatricNumber='". $_GET['studentID'] ."' AND schoolID='". $_GET['schoolID'] ."'";
							$result = mysqli_query($con, $sql);//or die(mysqli_error($con));
							if( mysqli_num_rows($con) > 0 )
							{
								while($row = mysqli_fetch_assoc($result))
								{
									...
									print("<td width='50%'>Matric Number :</td><td> <input type='text' name='txtMatricNumber' id='txtMatricNumber' value='".$row['studentMatricNumber']."' /><input type='hidden' name='hdnSchoolID' value='". $row['schoolID'] ."' /><input type='hidden' name='hdnStudentID' value='". $row['studentMatricNumber'] ."' /></td></tr>
											<td width='50%'>Email :</td><td> <input type='text' name='txtEmail' id='txtEmail' value='".$row['studentStudentEmail']."' /></td></tr>
											<td width='50%'>Supervisor :</td><td> <select name='cboSupervisor' id='soflow'>".$supervisors."</select></td></tr>
											
											");
								}
								print("<button class='button' name='btnSend' id='btnSend'>Send</button><button class='button' name='btnSend' id='btnSend'>Cancel</button>");
							}
		...
		<div class='outputContainer'>
				...
				<table border='1' width='100%' align='left'>
					<?php
						$count = 1;
						$pointer = "<img src='../images/pointer.png' width='10px' height='6px' />";
						$outPutPointer ="";
						//When the Matric Number of a student is clicked from the right-pane, the student history is tabulated in the gridview
						require_once '../connection.php';
						$sql = "SELECT * FROM tblStudents WHERE schoolID='". $_SESSION['schoolID'] ."' AND studentCompleted = 0";
						$result = mysqli_query($con, $sql);//or die(mysqli_error($con));
						if( mysqli_num_rows($con) > 0 )
						{
							while($row = mysqli_fetch_assoc($result))
							{
								...
								print("<tr><td width='5%'>".$count."</td><td width='50%'><a href='viewEditStudents.php?studentID=".
										$row['studentMatricNumber']."&schoolID=".$row['schoolID']."&supervisorID=".$row['studentSupervisor']."&action=Edit'>".$row['studentFirstName'] . " ".$row['studentLastName'] ."</td>".
										
										"<td width='40%' style='overflow:hide;'>".$row['studentEmail']."</td></tr>");
								$count += 1;
							}
      ...
      <div id="apDiv65"  style="border:1px #666666 solid;">
		...
		<div class='container'>
			...
			<div class='subTitle'>Welcome <?php print($_SESSION['schoolStaffName'] );  ?></div>
			<a href='../changePassword.php'><div class='menuButton'>Change Password</div></a>
			<a href='viewEditStudents.php'><div class='menuButton'>View/Edit Students</div></a>
			<a href='uploadStudents.php'><div class='menuButton'>Upload SIWES Students</div></a>
			<a href='addNewSchoolStaff.php'><div class='menuButton'>Add New Staff</div></a>
			<a href='viewEditAdminStaff.php'><div class='menuButton'>View/Edit Admin/Staff</div></a>
