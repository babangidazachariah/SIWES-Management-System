...
SESSION_START();
	if(!empty($_SESSION['matricNumber']))
	{
		...
		if(isset($_POST['btnSubmitButton'])){
			...
			if(empty($_POST['cboSelectBank']))
			{
				$isError = true;
			}
			if(empty($_POST['txtSortCode']))
			{
				$isError = true;
			}
			if(empty($_POST['txtAccountName']))
			{
				$isError = true;
			}
			if(empty($_POST['txtAccountNumber']))
			{
				$isError = true;
			}
			if (!($isError))
			{
				...
				$sql = "SELECT * FROM tblPaymentDetails WHERE studentMatricNumber='". $_SESSION['matricNumber'] ."'";
				$result = mysqli_query($con, $sql) or die(mysqli_error($con));
				
				if(mysqli_num_rows($result) > 0)
				{
					$sql = "UPDATE tblPaymentDetails SET paymentBankID ='".$_POST['cboSelectBank']."', 
															paymentBankSortCode ='".$_POST['txtSortCode']."', 
															paymentAccountNumber='".$_POST['txtAccountNumber']."', 
															paymentAccountName ='".$_POST['txtAccountName']."', 
															WHERE studentMatricNumber='". $_SESSION['matricNumber'] ."'";
					mysqli_query($con, $sql) or die(mysqli_error($con));
					$errorMessage = "Successfully Change Account Details.";
					
				}else{
					$sql = "INSERT INTO tblPaymentDetails (paymentBankID, 
															studentMatricNumber, 
															paymentBankSortCode, 
															paymentAccountNumber, 
															paymentAccountName) 
															VALUES ('".$_POST['cboSelectBank']."', 
															'". $_SESSION['matricNumber'] ."' 
															'".$_POST['txtSortCode']."', 
															'".$_POST['txtAccountNumber']."', 
															'".$_POST['txtAccountName']."')";
					mysqli_query($con, $sql) or die(mysqli_error($con));
					$errorMessage = "Successfully Received Account Details.";
				}
			}	
		}
...
<script type='text/Javascript'>
	...
	function ValidateSubmission()
	{
		...
		if((document.getElementById('txtSortCode').value == '') ) {
			errorMessage = "Type Bank Sort Code!!!";
		}
		if((document.getElementById('txtAccountNumber').value == '') ) {
			errorMessage = "Type Account Number!!!";
		}
		if((document.getElementById('txtAccountName').value == '') ) {
			errorMessage = "Type Account Name!!!";
		}
  ...
  <div id="mainContent">
    ...
    <div id="apDiv36">
      ...
      <div id="apDiv63">
		...
		<div class='container'>
			<h1 class="title">Bank Account Details</h1>
			<form id="form" class="form" action="bankAccountDetails.php" onsubmit="return ValidateSubmission();" method="POST">
				<label id='error' class='error'><?php print($errorMessage); ?></label>
				<select id="soflow"  name='cboSelectBank' tabindex="1">
				  <!-- This method is nice because it doesn't require extra div tags, but it also doesn't retain the style across all browsers. -->
				  <option value=''>Select Bank</option>
				  <option value='1'>Access Bank</option>
				  <option value='2'>Diamond Bank</option>
				  <option value='3'>Eco Bank</option>
				  <option value='4'>Fidelity Bank</option>
				  <option value='5'>First Bank</option>
				  <option value='6'>GT Bank</option>
				</select>
				<input type="text" placeholder="Bank Sort Code Here" id="txtSortCode" name="txtSortCode" tabindex="2"/>
			
				
				<input type="password" placeholder="Account Number Here" id="txtAccountNumber" name="txtAccountNumber" tabindex="3"/>
				
				<input type="password" placeholder="Account Name Here" id="txtAccountName" name="txtAccountName" tabindex="4"/>
      ...
      <div id="apDiv65"  style="border:1px #666666 solid;">
		...
		<div class='container'>
			...
			<div class='subTitle'>Welcome <?php print($_SESSION['firstName'] . " ".$_SESSION['lastName']);  ?></div>
			...
			<a href='siwesDailyReport.php'><div class='menuButton'>Daily SIWES Report</div></a>
