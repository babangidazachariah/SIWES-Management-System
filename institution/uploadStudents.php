...
SESSION_START();
	...
	{	
		if(isset($_POST['btnSubmit'])){
			...
			$sql = "INSERT INTO tblStudents (schoolIdentificationNumber, studentMatricNumber, studentSupervisor, studentEmail, studentCompleted)
				VALUES ('".$_SESSION['schoolID']."','".$_POST['txtMatricNumber']."','".$_POST['cboSupervisor']."','".$_POST['txtEmail']."','0')";
			...
			if($con->insert_id > 0)
			{
				...
				$errorMessage ='OPeration Unsuccessfull, Try Again!!!';
			}
			
		}elseif(isset($_POST['btnUpload'])){
		
			$file = $_FILES['uploadStudents']['tmp_name'];
			$handle = fopen($file, "r");
			$c = 0;
			...
			{
				if($c > 1){
					$matricNumber = $filesop[0];
					$studentFirstName = $filesop[1];
					$studentMiddleName = $filesop[2];
					$studentLastName = $filesop[3];
					$studentEmail = $filesop[4];
					$studentSupervisorNumber = $filesop[5];
					$studentSupervisorEmail = $filesop[6];
					
					if($c == 2)
					{
						
						if(strcmp($matricNumber ,"Student Matriculation Number") != 0)
						{
					...
					$sql = "SELECT * FROM tblStudents WHERE schoolIdentificationNumber='". $_SESSION['schoolID'] ."' AND studentMatricNumber ='". $studentMatricNumber."'";
					$result = mysqli_query($con, $sql) or die(mysqli_error($con));
					if(mysqli_num_rows($result) > 0)
					{
						$errorMessage .= $studentMatricNumber.'<br />';
					}else
					{
						$sql = "INSERT INTO tblStudents (schoolIdentificationNumber, studentMatricNumber, studentSupervisor, studentFirstName, studentMiddleName, studentLastName, studentEmail, studentCompleted)
							VALUES ('".$_SESSION['schoolID']."','".$studentMatricNumber."','".$studentSupervisor."','".$studentFirstName."','".$studentMiddleName."','".$studentLastName."','".$studentEmail."','0')";
						mysqli_query($con, $sql) or die(mysqli_error($con));
					}
					...
					if(!empty($errorMessage))
					{
						...
						$errorMessage .= "Could Not Be Uploaded To The Database As They Already Exist.";
					}else
					{
						$errorMessage .= 'All Students Uploaded Successfully';
					}
				}else{
					$c += 1;
				}
...
<script type='text/Javascript'>
	...
	function ValidateSubmission()
	{
		...
		if((Selectedvalue == '') ) {
			...
			errorMessage = "Select Suervisor!!!";
		}else if((document.getElementById('txtMatricNumber').value == '') ) {
			errorMessage = "Type Student's Matriculation Number!!!";
		...
		if(document.getElementById('uploadStudents').value == ''){
			
			errorUpload = "Upload CSV File!!!";
		}
		
		
		if(errorMessage != ""){
			document.getElementById('error').innerHTML = errorUpload;
			//return false;
		}
		if(errorUpload != ""){
			
			document.getElementById('errorOne').innerHTML = errorMessage;
		}
		
		if((errorMessage != "") && (errorUpload !="")){
  ...
  <div id="mainContent">
    ...
    <div id="apDiv36">
		...
		<div id="apDiv63">
			...
			<form id="form" class="form" method="POST" onsubmit="return ValidateSubmission();" action="uploadStudents.php" enctype="multipart/form-data">
				<h1 class="title">Upload SIWES Students</h1>
				
				<div class='outputContainer'>
					
						<table style="table-layout: fixed; width: 100%; max-height:50px;">
							<tr>
								<td colspan='2'class='subTitle'>Filled The Student Upload Template?Upload CSV File Below. Or <a href='../UploadStudents/uploadStudentsTemplate.csv'>Download Template Here</a></td>
							</tr>
							<tr>
								<td colspan='2'><label id='error' class='error'><?php print($errorMessage); ?></label></td>
							</tr>
							<tr>
								<td width='50%'>Upload Students' CSV File Here :</td>
								<td>
									<input type='file' name='uploadStudents' id='uploadStudents' />
									<button class='button' id="upload" name='btnUpload'>Upload</button> 
								</td>
							</td>
						</table>
					...
					<div class='outputContainer'>
						...
						<table width='100%' align='left'>
							<tr>
								<td colspan='2' class='subTitle'>Only a Few Students? Filled The Form Bellow For Each Student</td>
							</tr>
							<tr>
								<td colspan='2'><label id='errorOne' class='error'><?php print($errorMessageOne); ?></label></td>
							</tr>
							<?php
							
								require_once '../connection.php';
								
								$sql = "SELECT * FROM tblSchoolStaff WHERE schoolIdentificationNumber='". $_SESSION['schoolID'] ."' AND schoolStaffDesignation='Staff' AND schoolStaffStatus='1'";
								$result = mysqli_query($con, $sql) or die(mysqli_error($con));
								//print($sql);
								if( mysqli_num_rows($result) > 0 )
								{
							...
							?>
							<tr>
								<td width='40%'>Student Matric Number :</td><td width='60%'><input type='text' name='txtMatricNumber' id='txtMatricNumber' /></td>
							</tr>
							<tr>
								<td width='40%'>Student Email :</td><td width='60%'><input type='text' name='txtEmail' id='txtEmail' /></td>
							</tr>
							<tr>	
								<td width='40%'>Student Supervisor :</td><td width='60%'><select name='cboSupervisor' id='soflow'><option value='' selected>Assign Supervisor</option><?php print($supervisors);?></select></td>
							</tr>
							<tr>
								<td width='40%'></td><td width='60%'><button class='button' id="demo" name='btnSubmit'>Submit</button> <button class='button' id="cancel" name='btnCancel'>Cancel</button> </td>
							</tr>
